...
def make_common_model():
    ...
    wave_features = keras.layers.Conv1D(16, 9, activation='relu', padding='same')(wave_input)
    wave_features = keras.layers.Conv1D(32, 9, activation='relu', padding='same')(wave_features)
    wave_features = keras.layers.Conv1D(64, 9, activation='relu', padding='same')(wave_features)
    # wave_features = keras.layers.LayerNormalization()(wave_features)

    spectrogram_input = keras.layers.Input((None, 257, 1))
    spectrogram_features = keras.layers.Conv2D(8, (3, 3), activation='relu', padding='same')(spectrogram_input)
    spectrogram_features = keras.layers.Conv2D(4, (3, 3), activation='relu', padding='same')(spectrogram_features)
    spectrogram_features = keras.layers.Reshape((-1, 1028))(spectrogram_features)
    spectrogram_features = keras.layers.Conv1D(512, 3, activation='relu', padding='same')(spectrogram_features)
    spectrogram_features = keras.layers.Conv1D(64, 3, activation='relu', padding='same')(spectrogram_features)
    ...
    all_features = keras.layers.Conv1D(64, 9, activation='relu', padding='same', strides=4)(all_features)
    all_features = keras.layers.Conv1D(64, 9, activation='relu', padding='same', strides=4)(all_features)
    # all_features = keras.layers.LayerNormalization()(all_features)
    all_features = keras.layers.Conv1D(32, 9, activation='relu', padding='same', strides=4)(all_features)
